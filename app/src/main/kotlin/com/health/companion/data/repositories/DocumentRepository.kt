...
interface DocumentRepository {
    ...
    suspend fun getCachedDocuments(): List<DocumentResponse>
    suspend fun syncDocumentsToCache(docs: List<DocumentResponse>)
}
...
class DocumentRepositoryImpl @Inject constructor(
    ...
    override suspend fun getCachedDocuments(): List<DocumentResponse> {
        return try {
            documentDao.getAllDocuments().map { it.toDocumentResponse() }
        } catch (e: Exception) {
            Timber.w(e, "Failed to read cached documents")
            emptyList()
        }
    }

    override suspend fun syncDocumentsToCache(docs: List<DocumentResponse>) {
        try {
            val entities = docs.map { it.toCacheEntity() }
            documentDao.upsertAll(entities)
            if (docs.isNotEmpty()) {
                documentDao.deleteNotIn(docs.map { it.id })
            }
            Timber.d("Documents cache synced: ${docs.size} items")
        } catch (e: Exception) {
            Timber.w(e, "Failed to sync documents cache")
        }
    }
}

private fun DocumentEntity.toDocumentResponse() = DocumentResponse(
    id = id,
    filename = filename,
...
private fun DocumentResponse.toCacheEntity() = DocumentEntity(
    id = id,
    filename = filename,
    documentType = documentType ?: mimeType ?: "unknown",
    status = status ?: "unknown",
    filePath = "",
    uploadedAt = try {
        java.text.SimpleDateFormat("yyyy-MM-dd'T'HH:mm:ss", java.util.Locale.US)
            .parse(uploadedAt ?: "")?.time ?: System.currentTimeMillis()
